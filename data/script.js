...
async function updateData() {
    try {
        ...
        // Update status cards
        updateStatusCard('temperature', data.temperature, '°C', '🌡️', 18, 28);
        updateStatusCard('pressure', data.pressure, 'hPa', '🌬️', 1000, 1030);
        
        // Update system info
        document.getElementById('device-ip').textContent = data.wifi_ip || '--';
        document.getElementById('wifi-strength').textContent = data.wifi_rssi ? `${data.wifi_rssi} dBm` : '--';
        document.getElementById('uptime').textContent = formatUptime(data.uptime);
        document.getElementById('last-update').textContent = new Date().toLocaleTimeString();
        document.getElementById('memory-free').textContent = data.free_heap ? `${(data.free_heap/1024).toFixed(1)} KB` : '--';
        ...
        dataHistory.timestamps.push(new Date().toISOString());
        
        // Keep only last 24 hours (assuming 1 minute intervals)
        if (dataHistory.temperature.length > 1440) {
...
// Update status card with color coding
